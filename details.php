...
// checking viewed product 


//if(isset($_POST['recentlyviewed'])) {
...
$viewed_session = $_SESSION['customer_email'];

$get_customer_viewed = "select * from customers where customer_email='$viewed_session'";

$run_customer_viewed = mysqli_query($con,$get_customer_viewed);

$row_customer_viewed = mysqli_fetch_array($run_customer_viewed);

$customer_id_viewed = $row_customer_viewed['customer_id'];

//check duplicate entries and update timestamp 

$select_viewed = "select * from recentlyviewed where customer_id='$customer_id_viewed' AND product_id='$pro_id'";

$run_select = mysqli_query($con,$select_viewed);

$check_select = mysqli_num_rows($run_select);

if($check_select == 1){
$delete="delete from recentlyviewed where customer_id = '$customer_id_viewed' AND product_id = '$pro_id'";
//update timestamp
$run_update = mysqli_query($con,$delete);
$insert_again = "insert into recentlyviewed (customer_id,product_id) values ('$customer_id_viewed','$pro_id')";
$run_update_again = mysqli_query($con,$insert_again);

}
//checking duplicate ends
else {

$insert_viewed = "insert into recentlyviewed (customer_id,product_id) values ('$customer_id_viewed','$pro_id')";

$run_recentlyviewed = mysqli_query($con,$insert_viewed);
}
...
?>
						...
						<button class="tooltip">
                               ...
                               <button class="btn btn-primary"  type = "button" name="share" onclick="copyUrl()" onmouseout="outFunc()">
								<span class="tooltiptext" id="myTooltip">Share Link</span> <i class="fa fa-share"></i>
                                </button> </button>
								
								<button class="btn btn-primary" type="submit" name="pair" >

                                    <i class="fa fa-plus"></i> Pair	
...
echo "<script>alert('You Must Login To Add Product In Pair')</script>";
...
/* not doing duplicate check as of now...
$select_wishlist = "select * from pairing where customer_id='$customer_id' AND product_id='$pro_id'";
...
<script>
  ...
  var r = confirm('Press Ok to add this as second product of a pair or cancel to make it as first product of a new pair.');
  if (r == true) {
...
";*/
	...
	//if ($abcd==1){
	//$insert_pair_old = "insert into pairing (customer_id,product_2_id) values ('$customer_id','$pro_id')";
	$insert_pair_new = "insert into pairing (customer_id,product_1_id) values ('$customer_id','$pro_id')";

$run_wishlist = mysqli_query($con,$insert_pair_new);

if($run_wishlist){

 echo "<script> alert('Product Has Been Inserted as first product of a pair') </script>";
